...
const UseSygmaTo = (props: Props) => {
  const cardStyles = `relative border-2 border-gray-200 min-h-[250px] mt-2 py-5 px-3 flex flex-col gap-x-2 rounded-xl`;
  const cardHeadingStyles = `tracking-tight text-2xl text-gray-600 max-w-[80%]`;
  const cardHeadingStylesDarkMode = `tracking-tight text-2xl text-gray-100 max-w-[80%]`;
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="mx-auto mt-2 grid w-full grid-cols-1 justify-center lg:grid-cols-3 lg:items-center lg:space-x-4"
      >
        <motion.div variants={childVariants} className={cardStyles}>
          <h3 className={cardHeadingStyles}>Reduce fees for your userbase.</h3>
          <Man className="absolute bottom-5 right-1 w-[9rem] " />
        </motion.div>
        <motion.div
